...
export default async function addStaff(newProcessName, addToList, notificationFunc) {
  ...
  //console.log(clearTextFunc)
    ...
    const listItem = [newProcessName] 

   const attrToUpdate = "processinfo"
   ...
   //console.log(mapItem)
    ...
    if (response === 200 ) {
        notificationFunc("Added Process.")
        //dont display success here..wait 3 seconds for cloud
    } else {
        notificationFunc(`Error Adding Process.`)
    }
